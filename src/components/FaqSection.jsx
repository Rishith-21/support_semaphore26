...
export function FaqSection({ faqData, categories, globalSearch }) {
  ...
  const suggestions = useMemo(() => {
    ...
    const matchedHardcodedRules = ruleCategories.flatMap(cat =>
                ...
                {filteredRules.length > 0 && (
                  <div>
                    <div style={{
                      ...
                      padding: '1rem', borderRadius: '8px', borderLeft: '4px solid #10B981'
                    }}>
                      <IconAlertTriangle size={28} style={{ marginRight: '0.75rem', color: '#10B981' }} />
                      <h3 style={{ fontSize: '1.75rem', fontWeight: 800, color: '#1E293B', margin: 0 }}>
                      ...
                      {filteredRules.map(category => {
                        ...
                        return (
                          <div key={category.id} className="rule-card">
                            ...
                            >
                              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                                <IconAlertTriangle size={24} style={{ color: '#10B981', flexShrink: 0 }} />
                              ...
                              <ul className="rule-list" style={{ marginTop: '1.25rem' }}>
                                {category.rules.map((rule, idx) => (
                                  <li key={idx} className="rule-item">
                                    <IconCheck size={20} className="rule-icon" style={{ color: '#10B981' }} />
                                    <span>{rule}</span>
                                  </li>
                                ))}
                              </ul>
